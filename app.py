...
SUPABASE_PING_URL = (os.getenv("SUPABASE_PING_URL") or "").strip()

try:
    SUPABASE_PING_INTERVAL_SECONDS = int(os.getenv("SUPABASE_PING_INTERVAL_SECONDS", "21600").strip() or "21600")
except Exception:
    SUPABASE_PING_INTERVAL_SECONDS = 21600
...
class StatusWatcherBot(commands.Bot):
    def __init__(self):
        ...
        self._supabase_keepalive_task: asyncio.Task | None = None
        self._load_last_status()

    def _get_supabase_ping_url(self) -> str:
        if SUPABASE_PING_URL:
            return SUPABASE_PING_URL
        if not SUPABASE_URL:
            return ""
        base = SUPABASE_URL.rstrip("/")
        return f"{base}/rest/v1/users?select=user_id&limit=1"

    async def ping_supabase_loop(self):
        await self.wait_until_ready()

        if not SUPABASE_URL or not SUPABASE_KEY:
            log.warning("⚠️ SUPABASE_URL/SUPABASE_KEY not set; skipping Supabase keep-alive task.")
            return

        url = self._get_supabase_ping_url()
        if not url:
            log.warning("⚠️ SUPABASE ping URL is empty; skipping Supabase keep-alive task.")
            return
                    ...
                    async with session.get(url, headers=headers) as resp:
                        if resp.status == 200:
                            ...
                            try:
                                ...
                                body = ""
                            log.warning(f"⚠️ Supabase keep-alive ping failed: HTTP {resp.status}" + (f" body={body}" if body else ""))
            except Exception as e:
                log.error(f"❌ Supabase keep-alive ping error: {e}")

            await asyncio.sleep(max(300, int(SUPABASE_PING_INTERVAL_SECONDS)))
...
async def on_ready():
    ...
    if bot._supabase_keepalive_task is None or bot._supabase_keepalive_task.done():
        bot._supabase_keepalive_task = bot.loop.create_task(bot.ping_supabase_loop())
